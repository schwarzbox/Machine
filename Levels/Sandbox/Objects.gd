...
func __connect_child_signals(element: Element) -> void:
	...
	# warning-ignore:return_value_discarded
	element.connect("objects_has_selected_scene_received", self, "_on_Element_objects_has_selected_scene")
	# warning-ignore:return_value_discarded
...
func set_selected_elements(selected_areas: Array) -> void:
...
func __create_element(mouse_pos: Vector2) -> void:
	...
	if instance.type == Globals.ELEMENTS.WIRE:
		...
		if (
			...
			self._wire = instance
			self.call_deferred("__add_child", instance)
	else:
...
func _on_Element_objects_has_selected_scene(element: Element) -> void:
...
func _on_Element_selected_elements_moved(event: InputEvent) -> void:
	for element in self.get_selected_elements():
		...
		if not element.type == Globals.ELEMENTS.WIRE:
